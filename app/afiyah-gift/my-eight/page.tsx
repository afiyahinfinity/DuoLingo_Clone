...
export default function MyEightPage(){
  ...
  async function invite(slot:number,pillar:string){
    ...
    try{
      ...
      setLastLink(link);
      try{await navigator.clipboard.writeText(link);setMessage(`Invitation saved for ${value}. Her private join link has been copied.`)}
      catch{setMessage(`Invitation saved for ${value}. Copy the private join link below.`)}
      await load();
  ...
  return <main className="min-h-screen bg-[#061B26] px-4 py-8 text-[#F8F3E9]"><div className="mx-auto max-w-5xl">
    ...
    <section className="mt-5 overflow-hidden rounded-[32px] border border-[#AD8633]/30 bg-[radial-gradient(circle_at_top,#0c4a3a,#003629_45%,#061B26_100%)] p-6 lg:p-9">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><div className="text-xs font-bold uppercase tracking-[.25em] text-[#D6B46D]">∞ × ∞ · Multiplication</div><h1 className="mt-3 font-serif text-5xl">My Eight</h1><p className="mt-3 max-w-2xl text-sm leading-7 text-white/60">Invite eight real women by email. Each Sister receives her own private claim link, joins with her own Afiyah account, and only her agreed learning status appears here.</p></div><div className="rounded-3xl border border-[#D6B46D]/25 bg-black/20 px-5 py-4"><div className="flex items-center gap-2 text-[#D6B46D]"><Users className="h-5 w-5"/><strong>{rows.length} / 8 invited</strong></div><div className="mt-2 text-xs text-white/45">{joined} joined · {multiplied} multiplied · {openSlots} open</div></div></div>
    </section>
    ...
    <section className="mt-5 rounded-[28px] border border-[#D6B46D]/30 bg-[#003629]/55 p-5 lg:p-6">
      <div className="flex gap-4"><div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-[#AD8633]/15 text-[#D6B46D]"><Mail className="h-6 w-6"/></div><div><div className="text-xs font-bold uppercase tracking-[.2em] text-[#D6B46D]">Step 1 · Enter her email</div><h2 className="mt-1 font-serif text-3xl">Invite a Sister</h2><p className="mt-2 max-w-3xl text-sm leading-7 text-white/55">Choose an open pillar below, enter the Sister's email address, then tap <strong className="text-white/75">Send Sister Invite</strong>. Afiyah creates a private join link for that email. She must sign in or register using the same email before she can claim the slot.</p></div></div>
    </section>
